-- XBot schema bootstrap — combines drizzle migrations 0004 + 0005 + 0006 in an
-- idempotent form, for applying from the Neon console SQL editor when you can't
-- run `npm run db:push` locally. Safe to run repeatedly and safe on a database
-- where any subset of these migrations was already applied.
...
-- If a settings row already existed from Phase 1, column defaults above don't
-- change it. This adopts the growth-method pacing only where you kept the old
-- Phase 1 defaults (edited values are left alone):
